...
def _render_post_element(
    ...
    if tag == "at":
        mentioned_id = (
            ...
            or str(element.get("app_id", "")).strip()
        )
...
def normalize_feishu_message(*, message_type: str, raw_content: str) -> FeishuNormalizedMessage:
    ...
    if normalized_type == "image":
        ...
        return FeishuNormalizedMessage(
            ...
            relation_kind="image",
            metadata={"placeholder_text": FALLBACK_IMAGE_TEXT},
        )
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _message_mentions_bot(self, mentions: List[Any]) -> bool:
        ...
        for mention in mentions:
            ...
            mention_raw_id = mention_id.strip() if isinstance(mention_id, str) else None
            mention_open_id = getattr(mention_id, "open_id", None)
            mention_user_id = getattr(mention_id, "user_id", None)
            mention_app_id = getattr(mention_id, "app_id", None) or getattr(mention, "app_id", None)
            mention_name = (getattr(mention, "name", None) or "").strip()

            if self._app_id and (mention_raw_id == self._app_id or mention_app_id == self._app_id):
                return True
    ...
    def _post_mentions_bot(self, mentioned_ids: List[str]) -> bool:
        if not mentioned_ids:
            return False
        if self._app_id and self._app_id in mentioned_ids:
            return True
